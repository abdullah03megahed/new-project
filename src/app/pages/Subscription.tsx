...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    features: ['Up to 1,000 listings', 'Top listing placement', '90-day plan', 'Priority support', 'Advanced analytics'],
...
export const Subscription = () => {
  ...
  // ─── Subscribe: create subscription → open payment modal ─────────────────

  const handleSubscribe = async (plan: Plan) => {
    setSubscribing(plan.id);
    try {
      const subscriptionId = await api.post<number>(`/Subscription/${plan.id}`, {});
      if (!subscriptionId) {
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      if (result) {
        const currentPlan = PLANS.find(p => p.name.toLowerCase() === currentPlanName) || null;
        setPendingSubId(result);
        setPendingPlan(currentPlan);
        setPaymentOpen(true);
      } else {
        // Renew endpoint returned no ID — just confirm
  ...
  // ─── After payment succeeds, refresh subscription ─────────────────────────
  ...
  return (
    <div className="min-h-screen bg-[#B19CD9]/5">
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <Button variant="ghost" onClick={() => navigate('/dashboard')} className="mb-6 text-[#717182] hover:text-[#34495E]">
              ...
              >
                ...
                <CardContent className="space-y-4">
                  <div>
                    {plan.price === 0 ? (
                      ...
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-bold text-[#34495E]">EGP {plan.price.toLocaleString()}</span>
                        {plan.durationInDays && (
                  ...
                  <Badge className="text-white border-0" style={{ backgroundColor: plan.color }}>
                    {plan.maxListings >= 1000 ? `${plan.maxListings.toLocaleString()} listings` : `Up to ${plan.maxListings} listings`}
                  </Badge>
