...
import { Plus, Edit, Trash2, MapPin, Home, Bed, DollarSign } from 'lucide-react';
import {
...
interface DashboardStats {
  success: boolean;
  data: { totalListings: number; availableBeds: number; bookedBeds: number; startingFrom: number; };
}
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [listingsData, statsData] = await Promise.all([
          ...
          api.get<DashboardStats>('/landlord/dashboard-stats'),
        ]);
    ...
    if (user?.type === 'landlord') fetchAll();
  }, [user]);
    ...
    <div className="min-h-screen bg-[#B19CD9]/5">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={() => navigate('/add-house')} className="bg-[#FF6F61] hover:bg-[#FF6F61]/90 text-white">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-[#34495E]" style={{ fontSize: '24px', fontWeight: '700' }}>
                {stats?.startingFrom ? `EGP ${stats.startingFrom.toLocaleString()}` : '—'}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        {/* Listings */}
        <Card>
          <CardHeader><CardTitle className="text-[#34495E]">Your Properties</CardTitle></CardHeader>
          <CardContent>
            {loading ? (
              <div className="space-y-4">{[1, 2, 3].map(i => <div key={i} className="h-32 bg-gray-100 rounded-lg animate-pulse" />)}</div>
            ) : listings.length === 0 ? (
              <div className="text-center py-12">
                <Home className="w-16 h-16 text-[#717182] mx-auto mb-4" />
                <p className="text-[#717182] mb-4">No properties listed yet</p>
                <Button onClick={() => navigate('/add-house')} className="bg-[#00A5A7] hover:bg-[#00A5A7]/90 text-white">Add Your First Property</Button>
              </div>
            ) : (
              <div className="space-y-4">
                {listings.map((listing) => {
                  ...
                  const coverImage = listing.listingImages[0] ? `${IMAGE_BASE}${listing.listingImages[0]}` : null;

                  return (
                    <div key={listing.id} className="flex flex-col md:flex-row gap-4 p-4 border rounded-lg hover:border-[#00A5A7] transition-colors">
                      {coverImage ? (
                        <img src={coverImage} alt={listing.title} className="w-full md:w-48 h-32 object-cover rounded-lg" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      ) : (
                        <div className="w-full md:w-48 h-32 bg-gray-100 rounded-lg flex items-center justify-center"><Home className="w-8 h-8 text-gray-300" /></div>
                      )}
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Badge className={listing.status === 1 ? 'bg-[#B8E986] text-[#34495E] border-0' : 'bg-gray-100 text-gray-500 border-0'}>
                            {listing.status === 1 ? 'Active' : 'Inactive'}
                          </Badge>
                        </div>
                        <p className="text-[#717182] text-sm mb-3">{availBeds}/{totalBeds} beds available</p>
                        <div className="flex gap-2">
                          <Button onClick={() => navigate(`/add-house?edit=${listing.id}`)} variant="outline" size="sm" className="border-[#00A5A7] text-[#00A5A7]">
                            <Edit className="w-4 h-4 mr-2" />Edit
                          </Button>
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button variant="outline" size="sm" className="border-[#FF6F61] text-[#FF6F61] hover:bg-[#FF6F61] hover:text-white">
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDelete(listing.id)} className="bg-[#FF6F61] hover:bg-[#FF6F61]/90">Delete</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
